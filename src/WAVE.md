...
- **Wrong scale factors**: IMU registers use fixed-point. ±2g range = 0.000598 m/s² per LSB. ±250 dps = 0.00763 deg/s per LSB. Wrong values = wrong wave heights.
- **Gravity tracker drift**: The 0.02Hz low-pass gravity tracker takes ~50s to converge. The first ~500 samples are transient — that's why we collect 1800 samples but FFT uses only the last 1024.
...
```
IMU (10Hz, 3 min)
  → MPU6500: ax, ay, az, gx, gy, gz in physical units
  → Mahony AHRS filter (diagnostic, not used for heave)
...
FFT spectral analysis (last 1024 samples)
  ...
  → Tp = 1/f_peak  (frequency of max displacement PSD)
  → Power = 0.49 · Hs² · Tp  (deep-water approximation)
```

## Memory layout
- `accelBuf[1800]`: 7.2KB — heave acceleration samples, reused as FFT input
- `fftIm[1024]`: 4.1KB — imaginary part of FFT (static allocation)
- Total: ~11.3KB static RAM for wave processing

## Additional outputs
- **Mean tilt**: Angle between gravity vector and vertical, averaged over 3 min
...
- The Mahony filter is diagnostic-only — heave comes from the gravity tracker
